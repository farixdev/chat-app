...
class FirestoreService {
  ...
  Future<void> sendFriendRequest(
      FriendRequestModel request) async {
    try {
      await _firestore
  ...
  Future<void> respondToFriendRequst(
      String requestId,
    ...
    try {
      ...
      throw Exception(
          'Failed To Respond To Friend Request: ${e.toString()}');
    }
  }
  Stream<List<FriendRequestModel>> getFriendRequestsStream(String userId){
       ...
       .snapshots().map(
             (snapshot)=> snapshot.docs.map(
                 (doc)=> FriendRequestModel.fromMap(doc.data())).toList(),
         );
  }
  Stream<List<FriendRequestModel>> getSentFriendRequestStream(String userId){
       return _firestore.collection('friendsRequest')
  ...
  Future<FriendRequestModel?> getFriendRequest(
      ...
      String receiverId,
          ...
          try{
                QuerySnapshot query = await _firestore.collection('friendRequests').
                 ...
                 try{
                   ...
                   await createNotification(
                     NotificationModel(
                       ...
                       body: 'You have no longer  friend ',
                       createdAt: DateTime.now(),
                     ),
                   );
                   ...
                   throw Exception('failed to remove friend request :${e.toString()}');
                 }
    }
  Future<void> blockUser(String blockerId, String blockedId) async {
    try {
      ...
      throw Exception('failed to block friend :${e.toString()}');
    }
  }
  Future<void> unblockUser(String  user1Id, String user2Id) async {
    try {
      ...
      throw Exception('failed to unblock friend :${e.toString()}');
    }
  }
  Stream<List<FriendshipModel>> getFriendsStream(String userId) {
    return _firestore
      ...
      final friendships = snapshot.docs
          .map((doc) => FriendshipModel.fromMap(
  ...
  Future<FriendshipModel?> getFriendships(String user1Id,String user2Id) async{
           try{
             ...
             String friendShipId = '${userIds[0]}_${userIds[1]}';
             DocumentSnapshot doc = await _firestore.collection('friendships')
  ...
  Future<bool> isUserBlocked(String userId, String otherUserId) async {
    try {
      ...
      throw Exception(
          'failed to check if friend is blocked :${e.toString()}');
    }
  }
    ...
    try {
      ...
      if (!chatDoc.exists) {
        ChatModel newChat = ChatModel(
          ...
          lastSeenBy: {
            userId1: DateTime.now(),
            userId2: DateTime.now()
          },
          createdAt: DateTime.now(),
      ...
      throw Exception(
          'failed to create or get chat:${e.toString()}');
    }
  }
  Stream<List<ChatModel>> getUserChatsStream(String userId){
         return _firestore.collection('chats').where(
           'participants',arrayContains: userId,
         ).orderBy('updatedAt',descending: true).snapshots()
             .map((snapshot)=>snapshot.docs.map((doc)=>ChatModel.fromMap(doc.data()))
    ...
    Future<void> updateChatLastMessage(String chatId,MessageModel message) async{
           try{
             await _firestore.collection('chats').doc(
                chatId
             ).update({'LastMessage': message.content,
             'LastMessageTimer': message.timeStamp.millisecondsSinceEpoch,
             'lastMessageSenderId': message.senderId,
               'updateAt': DateTime.now().millisecondsSinceEpoch,
             });
           } catch(e){
             throw Exception('failed to update chat list:${e.toString()}');
           }
           }
           Future<void> updateUserLastSeen(String chatId,String userId)async{
                      try{
                        await _firestore.collection('chats').doc(
                            chatId
                        ).update({'LastSeenBy.$userId':DateTime.now().millisecondsSinceEpoch,
                        });
  ...
  Future<void> sendMessage(MessageModel message)async{
    try{
      await _firestore.collection('messages').doc(
          message.id
      ).set(message.toMap());
      ...
      DocumentSnapshot  chatDoc = await _firestore.collection('chats').doc(chatId)
      .get();
            ...
            for (var doc in snapshot.docs) {
              ...
              if ((message.senderId == userId1 && message.receiverId == userId2) ||
                  (message.senderId == userId2 && message.receiverId == userId1)) {
                    ...
                    if(chat != null){
                        DateTime? currentUserDeleteAt=chat.getDeletedAt(userId1);
                        if(currentUserDeleteAt != null && message.timeStamp.isBefore(currentUserDeleteAt)){
      ...
      try{
        return _firestore.collection('notifications')
            ...
            .snapshots()
             .map(
            (snapshot)=>
                       snapshot.docs.map((doc)=> NotificationModel.fromMap(doc.data())).
                      toList(),
        );
      }
      catch(e){
  ...
  Future<void> markAllNotificationAsRead(String userId) async{
    try{
      ...
      await _firestore.collection('notifications').where('userId', isEqualTo: userId)
  ...
  Future<void> deleteNotificationsByTypeAndUser(
    ...
    try{
         ...
         for(var doc in notifications.docs){
                ...
                if(data['data'] !=null &&
                    (data['data']['sendId']== relatedUserId
      ...
      print('error deleting notification:$e');
           ...
           Future<void> _removeNotificationForCancelRequest(
               ...
               String senderId,
                    ...
                    try{
                        await deleteNotificationsByTypeAndUser(receiverId,
                          NotificationType.friendRequest,
                          senderId,
                        );
                    }
                 ...
                 //     throw Exception('failed to delete notification:${e.toString()}');
                      print('error removing  notification and cancel request:$e');
                    }
