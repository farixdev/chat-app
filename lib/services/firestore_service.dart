...
class FirestoreService {
  ...
  Stream<List<FriendshipModel>> getFriendsStream(String userId) {
    return _firestore.collection('friendships').snapshots().map((snapshot) {
      final friendships = snapshot.docs
          .map((doc) => FriendshipModel.fromMap(doc.data()))
          .where(
            (f) => (f.user1Id == userId || f.user2Id == userId) && !f.isBlocked,
          )
          .toList();
  ...
  Future<bool> isUserBlocked(String userId, String otherUserId) async {
    try {
      ...
      throw Exception('failed to check if friend is blocked :${e.toString()}');
    }
  }
